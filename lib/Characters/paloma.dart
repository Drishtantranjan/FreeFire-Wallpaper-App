...
import '../agentOverview/palomaOverview.dart';

class Paloma extends StatefulWidget {
  const Paloma({Key? key});

  @override
  State<Paloma> createState() => _PalomaState();
}

class _PalomaState extends State<Paloma> {
  Future<List<String>> fetchImages() async {
    final QuerySnapshot snapshot = await FirebaseFirestore.instance
        .collection('paloma') // Replace 'paloma' with your collection name
        .get();
      ...
      return GestureDetector(
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => PalomaOverview(imageUrl: imageUrls[index]),
            ),
          );
        },
        child: Container(
