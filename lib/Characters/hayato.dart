...
import '../agentOverview/hayatoOverview.dart';

class Hayato extends StatefulWidget {
  const Hayato({Key? key});

  @override
  State<Hayato> createState() => _HayatoState();
}

class _HayatoState extends State<Hayato> {
  Future<List<String>> fetchImages() async {
    final QuerySnapshot snapshot = await FirebaseFirestore.instance
        .collection('hayato') // Replace 'hayato' with your collection name
        .get();
      ...
      return GestureDetector(
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => HayatoOverview(imageUrl: imageUrls[index]),
            ),
          );
        },
        child: Container(
