...
import '../agentOverview/klaOverview.dart';

class Kla extends StatefulWidget {
  const Kla({Key? key});

  @override
  State<Kla> createState() => _KlaState();
}

class _KlaState extends State<Kla> {
  Future<List<String>> fetchImages() async {
    final QuerySnapshot snapshot = await FirebaseFirestore.instance
        .collection('kla') // Replace 'kla' with your collection name
        .get();
      ...
      return GestureDetector(
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => KlaOverview(imageUrl: imageUrls[index]),
            ),
          );
        },
        child: Container(
