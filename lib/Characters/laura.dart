...
import '../agentOverview/lauraOverview.dart';

class Laura extends StatefulWidget {
  const Laura({Key? key});

  @override
  State<Laura> createState() => _LauraState();
}

class _LauraState extends State<Laura> {
  Future<List<String>> fetchImages() async {
    final QuerySnapshot snapshot = await FirebaseFirestore.instance
        .collection('laura') // Replace 'laura' with your collection name
        .get();
      ...
      return GestureDetector(
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => LauraOverview(imageUrl: imageUrls[index]),
            ),
          );
        },
        child: Container(
